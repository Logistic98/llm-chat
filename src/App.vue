        ...
        <div class="chat_left_body">
          <div v-for="(item, index) in messages" :key="'record_card' + index" :class="item.id === currentId ? 'record_card card_choose' : 'record_card'" @click="recordCardClick(item)">
            <div class="record_card_title">
              ...
              <Input v-else v-model="item.name" placeholder="请输入名称" style="width: 150px" @change="saveMessages"></Input>
              <Button shape="circle" type="primary" icon="md-create" size="small" style="position: absolute;top:7px;right: 38px;height: 26px;width: 26px" v-if="!item.edit" @click.stop="changeMessageEditStatus(item, true)"></Button>
              <Button shape="circle" type="success" icon="md-checkmark" size="small" style="position: absolute;top:7px;right: 38px;height: 26px;width: 26px" v-else @click.stop="changeMessageEditStatus(item, false)"></Button>
              <Button shape="circle" type="error" icon="ios-trash" size="small" style="position: absolute;top:7px;right: 7px;height: 26px;width: 26px" @click.stop="deleteRecord(index, item.id)"></Button>
            </div>
        ...
        <div class="chat_right_body" ref="messages">
          <div v-for="(message, index) in currentRecord.value" :key="'message' + index" :class="message.isSelf ? 'message self' : 'message'">
            ...
            <div :class="message.isSelf ? 'content self' : 'content'">
              ...
              <div style="margin: auto 0 auto auto;" v-if="!message.isSelf">
                <span style="margin: 10px">渲染</span>
                <i-switch   v-model="message.isMarkdown" size="default">
                </i-switch>
              </div>
...
export default {
  ...
  mounted () {
    ...
    // 打开一个页面时首先看浏览器中是否存有这个大屏的历史信息
    this.storageKey = 'chatInfo'
    if (localStorage.getItem(this.storageKey) !== null) { // 如果有存储历史信息的话取出
  ...
  data () {
    return {
      ...
      options: {
        apiUrl: 'https://openai.eula.club/v1/chat/completions',
        api_key: 'sk-xxx',
        serverName: '机器人',
        userName: '我',
    ...
    sendMessage () {
      ...
      fetch(this.options.apiUrl,{
        ...
        return reader.read().then(result=>{
          if (chooseRecord.value[lastIndex].text !== '已停止生成' && !result.done) {
            let value = JSON.parse(new TextDecoder().decode(result.value))
            chooseRecord.value[lastIndex].text += value['choices'][0]['message']['content']
            chooseRecord.value[lastIndex].loading = false
            chooseRecord.sending = false
            console.log("chooseRecord: ", chooseRecord)
            this.saveMessages()
          }
        }).catch(function(err) {
          console.error(err);
